...
import com.alpha.networkfile.rclone.COMMAND_CONFIG
import com.alpha.networkfile.rclone.COMMAND_COPY
import com.alpha.networkfile.rclone.COMMAND_DELETE
import com.alpha.networkfile.rclone.COMMAND_LSJSON
import com.alpha.networkfile.rclone.COMMAND_OBSCURE
import com.alpha.networkfile.rclone.COMMAND_SERVE
import com.alpha.networkfile.rclone.OAUTH_PROCESS_REGEX
import com.alpha.networkfile.rclone.Rclone
import com.alpha.networkfile.rclone.entity.About
...
import com.alpha.networkfile.rclone.entity.SpaceInfo
...
const val TAG = "AndroidRcx"
const val NATIVE_LIB_NAME = "librclone.so"

class AndroidRclone(context: Context): Rclone {

  override val downloadScope = Executors.newFixedThreadPool(3).asCoroutineDispatcher()

  override val rClone = context.applicationInfo.nativeLibraryDir + "/$NATIVE_LIB_NAME"
  override val rCloneConfig = context.filesDir.path + "/rclone.conf"
  override val logFilePath = context.filesDir.path + "/rclone.log"
  override val serveLogFilePath = context.filesDir.path + "/rclone_serve.log"
  ...
  override fun logErrorOut(process: Process): String {
...
//    val localRemotePath = if (remote.isType(LOCAL)) getLocalPathPrefix(remote) + "/" else ""
  ...
  //  fun uploadFile(remote: RemoteStorage, uploadPath: String, uploadFile: String): Process
  //
  //  fun deleteFile(remote: RemoteStorage, file: RcloneFileItem): Process
  //
  //  fun mkdir(remote: RemoteStorage, path: String): Boolean
  //
  //  fun moveTo(remote: RemoteStorage, file: RcloneFileItem, newPath: String): Process
  //
  //  fun moveTo(remote: RemoteStorage, oldFile: String, newFile: String): Boolean
}
